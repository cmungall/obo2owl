...
\pr{expandAssertionTo}(?R,?V)  $\Rightarrow$ substitute(?R(?X,?Y),?V)
\end{expr}
...
\pr{expandExpressionTo}(?R,?V)  $\Rightarrow$ substitute(?R some ?Y,?V)
\end{expr}
...
\begin{expr}\label{exp-some}
\pr{expandExpressionTo}(?R,?V)  $\Rightarrow$ substitute(?R value ?Y,?V)
\end{expr}
